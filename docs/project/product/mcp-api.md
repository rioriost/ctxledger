...
- when summaries are enabled and returned, the response may explicitly mark summary-first assembly through:
  ...
  - in multi-workflow workspace- or ticket-resolved cases, that same summary-only low-limit query-filter reading should still currently keep grouped summary `parent_scope_id = null` even when one visible episode remains
  ...
  - in multi-workflow workspace- or ticket-resolved summary-only low-limit query-filter cases, that same reading should also not be taken to strengthen grouped summary parentage merely because one visible episode remains
