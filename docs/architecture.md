...
Repository projections are derived artifacts intended to help agents and tools discover current state.
...
Projection failure is operationally significant and must be recorded durably in PostgreSQL.

Projection failure records should capture enough metadata for:

- diagnosis
- retry
- eventual resolution tracking
