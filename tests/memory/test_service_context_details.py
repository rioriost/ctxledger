...
def test_memory_get_context_task_recall_keeps_detour_penalty_when_objective_text_is_detour_like() -> (
    ...
    assert response.details["task_recall_latest_vs_selected_checkpoint_details_present"] is True
    assert response.details["task_recall_latest_vs_selected_checkpoint_details"] == {
        "latest_workflow_instance_id": str(detour_like_objective_workflow_id),
        "selected_workflow_instance_id": str(detour_like_objective_workflow_id),
        "latest_considered": {
            "workflow_instance_id": str(detour_like_objective_workflow_id),
            "checkpoint_step_name": "coverage follow-up",
            "checkpoint_summary": "Coverage cleanup after the main change",
            "primary_objective_text": "Coverage cleanup and docs review",
            "next_intended_action_text": None,
            "ticket_detour_like": False,
...
def test_memory_get_context_workspace_only_low_limit_query_filter_summary_first_keeps_surviving_child_set_when_memory_items_disabled() -> (
    None
):
    first_workflow_id = uuid4()
    second_workflow_id = uuid4()
    ...
    matching_episode = EpisodeRecord(
        episode_id=uuid4(),
        workflow_instance_id=first_workflow_id,
        summary="Workspace-only low-limit summary-only surviving query match",
        metadata={"kind": "matching"},
        created_at=created_at.replace(day=27),
        updated_at=created_at.replace(day=27),
    )
    filtered_episode = EpisodeRecord(
        episode_id=uuid4(),
        workflow_instance_id=second_workflow_id,
        summary="Workspace-only low-limit summary-only filtered out by query",
        metadata={"kind": "filtered"},
        created_at=created_at.replace(day=26),
        updated_at=created_at.replace(day=26),
    )
    ...
    matching_memory_item = MemoryItemRecord(
        ...
        type="episode_note",
        provenance="episode",
        content="Matching workspace-only low-limit summary-only memory item",
        metadata={"kind": "matching-note"},
        created_at=created_at.replace(day=27, hour=2),
        updated_at=created_at.replace(day=27, hour=2),
    )
    filtered_memory_item = MemoryItemRecord(
        ...
        type="episode_note",
        provenance="episode",
        content="Filtered workspace-only low-limit summary-only memory item",
        metadata={"kind": "filtered-note"},
        created_at=created_at.replace(day=26, hour=2),
        updated_at=created_at.replace(day=26, hour=2),
    )
    ...
    service = MemoryService(
        ...
        workflow_lookup=InMemoryWorkflowLookupRepository(
            workflows_by_id={
                first_workflow_id: {
                    "workspace_id": workspace_id,
                    "ticket_id": "TICKET-CONTEXT-WORKSPACE-ONLY-MULTI-WORKFLOW-LIMIT-QUERY-SUMMARY-ONLY",
                },
                second_workflow_id: {
                    "workspace_id": workspace_id,
                    "ticket_id": "TICKET-CONTEXT-WORKSPACE-ONLY-MULTI-WORKFLOW-LIMIT-QUERY-SUMMARY-ONLY-OTHER",
                },
            }
        ),
    )
    ...
    assert [episode.summary for episode in response.episodes] == [
        "Workspace-only low-limit summary-only surviving query match",
    ]
    assert response.details["query_filter_applied"] is True
    assert response.details["episodes_before_query_filter"] == 1
    assert response.details["matched_episode_count"] == 1
