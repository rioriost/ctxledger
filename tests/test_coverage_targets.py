...
def test_in_memory_projection_state_repository_prefers_recent_projection_sort_signals() -> (
    None
):
    from datetime import UTC, datetime
    ...
    repo = InMemoryProjectionStateRepository(
        {
            ...
            (
                ...
                status=ProjectionStatus.FRESH,
                target_path=".agent/resume.md",
                last_successful_write_at=datetime(2024, 1, 3, tzinfo=UTC),
                last_canonical_update_at=datetime(2024, 1, 3, tzinfo=UTC),
                open_failure_count=0,
            ),
        }
    )
...
def test_in_memory_projection_failure_repository_resolve_and_ignore_with_missing_entries() -> (
    None
):
    from ctxledger.db import InMemoryProjectionFailureRepository

    repo = InMemoryProjectionFailureRepository({}, {})
    ...
    assert repo.ignore_resume_projection_failures(uuid4(), uuid4()) == 0
...
def test_build_embedding_generator_supports_all_external_provider_branches() -> None:
    voyage_generator = build_embedding_generator(
        EmbeddingSettings(
    ...
    cohere_generator = build_embedding_generator(
        EmbeddingSettings(
    ...
    custom_generator = build_embedding_generator(
        EmbeddingSettings(
            provider=EmbeddingProvider.CUSTOM_HTTP,
            model="custom-model",
            api_key="secret",
    ...
    assert isinstance(voyage_generator, ExternalAPIEmbeddingGenerator)
    assert voyage_generator.base_url == "https://api.voyageai.com/v1"
    assert isinstance(cohere_generator, ExternalAPIEmbeddingGenerator)
    assert cohere_generator.base_url == "https://api.cohere.com/v1"
    assert isinstance(custom_generator, ExternalAPIEmbeddingGenerator)
    assert custom_generator.base_url == "https://embeddings.example.com/v1"


def test_external_embedding_generator_rejects_unimplemented_provider_after_validation() -> (
    None
):
    generator = ExternalAPIEmbeddingGenerator(
        provider=EmbeddingProvider.VOYAGEAI,
...
def test_extract_embedding_vector_rejects_non_mapping_payload() -> None:
    with pytest.raises(EmbeddingGenerationError) as exc_info:
        from ctxledger.memory.embeddings import _extract_embedding_vector

        _extract_embedding_vector("not-a-dict", provider="custom_http")

    assert exc_info.value.details == {"field": "response"}


def test_extract_embedding_vector_accepts_vector_key_and_nested_vector_key() -> None:
    from ctxledger.memory.embeddings import _extract_embedding_vector

    assert _extract_embedding_vector(
        {"vector": [1, 2.5, -3]},
        provider="custom_http",
    ) == (1.0, 2.5, -3.0)
    assert _extract_embedding_vector(
        {"data": [{"vector": [0.25, -0.5, 1]}]},
        provider="custom_http",
    ) == (0.25, -0.5, 1.0)


def test_external_embedding_generator_reports_url_error_details() -> None:
    generator = ExternalAPIEmbeddingGenerator(
        provider=EmbeddingProvider.CUSTOM_HTTP,
        model="custom-model",
        api_key="secret",
...
def test_stable_metadata_returns_empty_dict_for_none() -> None:
    from ctxledger.memory.embeddings import _stable_metadata

    assert _stable_metadata(None) == {}


def test_extract_response_model_falls_back_for_non_mapping_and_blank_model() -> None:
    from ctxledger.memory.embeddings import _extract_response_model

    assert (
        _extract_response_model("not-a-dict", default="fallback-model")
        == "fallback-model"
    )
    assert (
        _extract_response_model({"model": "   "}, default="fallback-model")
        == "fallback-model"
    )
    assert (
...
def test_normalize_embedding_vector_rejects_non_numeric_entries() -> None:
    from ctxledger.memory.embeddings import _normalize_embedding_vector

    assert _normalize_embedding_vector([1, "bad", 3]) is None


def test_normalize_text_rejects_non_string_input() -> None:
    from ctxledger.memory.embeddings import _normalize_text

    assert _normalize_text(123) == ""  # type: ignore[arg-type]


def test_hash_to_vector_rejects_non_positive_dimensions() -> None:
    from ctxledger.memory.embeddings import _hash_to_vector

    with pytest.raises(EmbeddingGenerationError) as exc_info:
        _hash_to_vector("00" * 32, dimensions=0)

    assert exc_info.value.details == {"dimensions": 0}
...
def test_in_memory_memory_episode_item_and_embedding_repositories_cover_create_and_list() -> (
    None
):
    from ctxledger.db import (
        InMemoryMemoryEmbeddingRepository as DbEmbeddingRepo,
    )
    from ctxledger.db import (
        InMemoryMemoryEpisodeRepository,
    )
    from ctxledger.db import (
        InMemoryMemoryItemRepository as DbItemRepo,
    )
    ...
    newer_episode = EpisodeRecord(
        episode_id=uuid4(),
        workflow_instance_id=workflow_id,
        summary="Newer episode summary",
        created_at=datetime(2024, 1, 3, tzinfo=UTC),
        updated_at=datetime(2024, 1, 3, tzinfo=UTC),
    )
    episode_repo = InMemoryMemoryEpisodeRepository({})
    episode_repo.create(episode)
    episode_repo.create(newer_episode)

    assert episode_repo.list_by_workflow_id(workflow_id, limit=5) == (
        newer_episode,
        episode,
    )
    ...
    item_repo = DbItemRepo({})
    item_repo.create(older_item)
    item_repo.create(newer_item)

    assert item_repo.list_by_workspace_id(workspace_id, limit=5) == (
        newer_item,
        older_item,
    )
    assert item_repo.list_by_episode_id(episode.episode_id, limit=5) == (
        newer_item,
        older_item,
    )
    ...
    older_embedding = MemoryEmbeddingRecord(
        memory_embedding_id=uuid4(),
        memory_id=older_item.memory_id,
    ...
    embedding_repo = DbEmbeddingRepo({})
    embedding_repo.create(older_embedding)
    embedding_repo.create(newer_embedding)

    assert embedding_repo.list_by_memory_id(older_item.memory_id, limit=5) == (
        newer_embedding,
        older_embedding,
    )


def test_memory_service_error_string_and_not_implemented_helper() -> None:
    service = MemoryService()
    ...
    response = service._not_implemented(
        feature=MemoryFeature.SEARCH,
        message="Not ready",
        details={"reason": "later"},
    )

    assert response.feature is MemoryFeature.SEARCH
    assert response.implemented is False
    assert response.message == "Not ready"
    assert response.available_in_version is None
    assert response.details == {"reason": "later"}
...
def test_unit_of_work_lookup_repository_handles_missing_workflow_and_projection_absence() -> (
    ...
    assert freshness["latest_projection_canonical_update_at"] is None
    assert freshness["latest_projection_successful_write_at"] is None
    assert freshness["projection_open_failure_count"] == 0
...
def test_embedding_dot_product_returns_zero_for_dimension_mismatch() -> None:
    assert _embedding_dot_product((1.0,), (1.0, 2.0)) == 0.0
...
def test_in_memory_memory_embedding_repository_list_by_memory_id_orders_recent_first() -> (
    None
):
    repo = InMemoryMemoryEmbeddingRepository()
    memory_id = uuid4()
    older = MemoryEmbeddingRecord(
        memory_embedding_id=uuid4(),
        memory_id=memory_id,
        embedding_model="test",
        embedding=(0.1, 0.2),
    ...
    assert repo.list_by_memory_id(memory_id, limit=5) == (newer, older)


def test_memory_service_collect_episode_context_handles_empty_workflow_ids_and_duplicates() -> (
    None
):
    service = MemoryService()

    assert service._collect_episode_context(workflow_ids=(), limit=5) == ()
    ...
    class DuplicateEpisodeRepository:
        def list_by_workflow_id(
            self,
            workflow_instance_id: UUID,
            *,
            ...
            return (shared_episode, newer_episode)

    service = MemoryService(episode_repository=DuplicateEpisodeRepository())

    collected = service._collect_episode_context(
        workflow_ids=(workflow_id, workflow_id),
        limit=5,
    )

    assert collected == (newer_episode, shared_episode)


def test_memory_service_build_episode_explanations_with_summary_and_metadata_miss() -> (
    None
):
    service = MemoryService()
    episode = EpisodeRecord(
...
def test_memory_service_score_memory_item_for_query_with_metadata_value_and_no_match() -> (
    None
):
    service = MemoryService()
    memory_item = MemoryItemRecord(
    ...
    metadata_value_score, metadata_value_fields = service._score_memory_item_for_query(
        memory_item=memory_item,
        normalized_query="postgres",
    )
    no_match_score, no_match_fields = service._score_memory_item_for_query(
        memory_item=memory_item,
        normalized_query="missing",
    )

    assert metadata_value_score == 1.5
    assert metadata_value_fields == ("metadata_values",)
    assert no_match_score == 0.0
    assert no_match_fields == ()


def test_memory_service_parse_uuid_accepts_attribute_error_path() -> None:
    service = MemoryService()
    ...
    assert exc_info.value.code is MemoryErrorCode.INVALID_REQUEST


def test_memory_service_maybe_store_embedding_records_success_and_failure_variants() -> (
    None
):
    memory_item = MemoryItemRecord(
        memory_id=uuid4(),
    ...
    recording_generator = RecordingEmbeddingGenerator()
    embedding_repo = InMemoryMemoryEmbeddingRepository()
    service = MemoryService(
        embedding_generator=recording_generator,
        memory_embedding_repository=embedding_repo,
    )

    success_details = service._maybe_store_embedding(memory_item)
    ...
    assert success_details == {
        ...
        "embedding_content_hash": "hash-123",
    }
    assert recording_generator.requests == [
        EmbeddingRequest(
    ...
    failure_service = MemoryService(
        embedding_generator=FailingEmbeddingGenerator(),
        memory_embedding_repository=InMemoryMemoryEmbeddingRepository(),
    )

    failure_details = failure_service._maybe_store_embedding(memory_item)
        ...
        workflow_lookup=InMemoryWorkflowLookupRepository(
            workflows_by_id={
                checkpoint_fresh_workflow_id: {
                    ...
                    "latest_projection_canonical_update_at": created_at.replace(day=14),
                    "latest_projection_successful_write_at": created_at.replace(day=13),
    ...
    assert response.details["workflow_candidate_ordering"] == {
        ...
        "candidate_signals": {
            str(checkpoint_fresh_workflow_id): {
                ...
                "latest_projection_canonical_update_at": (
                    created_at.replace(day=14).isoformat()
                ),
                "latest_projection_successful_write_at": (
                    created_at.replace(day=13).isoformat()
                ),
                "projection_open_failure_count": 1,
                "latest_episode_created_at": (created_at.replace(day=10).isoformat()),
    ...
    assert response.details["workflow_candidate_ordering"] == {
        ...
        "candidate_signals": {
            str(verify_fresh_workflow_id): {
                ...
                "latest_projection_canonical_update_at": (
                    created_at.replace(day=7).isoformat()
                ),
                "latest_projection_successful_write_at": (
                    created_at.replace(day=7).isoformat()
                ),
            ...
            str(verify_stale_workflow_id): {
                ...
                "latest_projection_canonical_update_at": (
                    created_at.replace(day=8).isoformat()
                ),
                "latest_projection_successful_write_at": (
                    created_at.replace(day=8).isoformat()
                ),
...
def test_memory_get_context_prefers_projection_freshness_after_checkpoint_and_verify_tie() -> (
    None
):
    projection_fresh_workflow_id = uuid4()
    projection_stale_workflow_id = uuid4()
    ...
    episode_repository.create(
        EpisodeRecord(
            episode_id=uuid4(),
            workflow_instance_id=projection_fresh_workflow_id,
            summary="Projection-fresh workflow",
            metadata={"kind": "projection-fresh"},
            created_at=created_at.replace(day=2),
            updated_at=created_at.replace(day=2),
        )
    )
    episode_repository.create(
        EpisodeRecord(
            episode_id=uuid4(),
            workflow_instance_id=projection_stale_workflow_id,
            summary="Projection-stale workflow",
            metadata={"kind": "projection-stale"},
    ...
    service = MemoryService(
        ...
        workflow_lookup=InMemoryWorkflowLookupRepository(
            workflows_by_id={
                projection_fresh_workflow_id: {
                    "workspace_id": "00000000-0000-0000-0000-000000000023",
                    "ticket_id": "TICKET-PROJECTION",
                    "latest_checkpoint_created_at": created_at.replace(day=10),
                    "latest_verify_report_created_at": created_at.replace(day=11),
                    "latest_projection_canonical_update_at": created_at.replace(day=12),
                    "latest_projection_successful_write_at": created_at.replace(day=11),
                    "projection_open_failure_count": 2,
                    "latest_attempt_started_at": created_at.replace(day=9),
                    "workflow_updated_at": created_at.replace(day=9),
                },
                projection_stale_workflow_id: {
                    "workspace_id": "00000000-0000-0000-0000-000000000023",
                    "ticket_id": "TICKET-PROJECTION",
                    "latest_checkpoint_created_at": created_at.replace(day=10),
                    "latest_verify_report_created_at": created_at.replace(day=11),
    ...
    assert response.details["workflow_candidate_ordering"] == {
        ...
        "workspace_candidate_ids": [
            str(projection_fresh_workflow_id),
            str(projection_stale_workflow_id),
        ],
        "ticket_candidate_ids": [],
        "resolver_candidate_ids": [
            str(projection_fresh_workflow_id),
...
def test_memory_get_context_prefers_non_terminal_workflow_over_terminal_workflow() -> (
    ...
    running_workflow_id = uuid4()
    terminal_workflow_id = uuid4()
    ...
    episode_repository.create(
        EpisodeRecord(
            episode_id=uuid4(),
            workflow_instance_id=running_workflow_id,
            summary="Running workflow context",
            metadata={"kind": "running"},
            created_at=created_at.replace(day=2),
            updated_at=created_at.replace(day=2),
        )
    )
    episode_repository.create(
        EpisodeRecord(
            episode_id=uuid4(),
            workflow_instance_id=terminal_workflow_id,
            summary="Terminal workflow context",
        ...
        workflow_lookup=InMemoryWorkflowLookupRepository(
            workflows_by_id={
                running_workflow_id: {
                    ...
                    "latest_verify_report_created_at": created_at.replace(day=4),
                    "latest_projection_canonical_update_at": created_at.replace(day=4),
                    "latest_projection_successful_write_at": created_at.replace(day=4),
                    "projection_open_failure_count": 0,
                    "latest_attempt_started_at": created_at.replace(day=5),
                    "workflow_updated_at": created_at.replace(day=5),
                },
                terminal_workflow_id: {
                    ...
                    "latest_verify_report_created_at": created_at.replace(day=6),
                    "latest_projection_canonical_update_at": created_at.replace(day=6),
                    "latest_projection_successful_write_at": created_at.replace(day=6),
                    "projection_open_failure_count": 0,
    ...
    assert response.details["workflow_candidate_ordering"] == {
        ...
        "workspace_candidate_ids": [
            str(running_workflow_id),
            str(terminal_workflow_id),
        ],
        "ticket_candidate_ids": [],
        "resolver_candidate_ids": [
            str(running_workflow_id),
        ...
        "candidate_signals": {
            str(running_workflow_id): {
                ...
                "workflow_updated_at": created_at.replace(day=5).isoformat(),
            },
            str(terminal_workflow_id): {
                "workflow_status": "completed",
                "workflow_is_terminal": True,
