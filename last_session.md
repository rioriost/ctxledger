Patch 2 の scaffold 実装、MCP RPC extraction の一部、stdio responsibility split、stdio builder extraction、そして stdio runtime bootstrap helper split まで進めました。
...
- stdio 削除前に、builder / wiring / runtime / server / bootstrap を段階的に分離する
...
- `create_runtime(...)` に残る stdio-specific runtime construction 分岐はまだ `server.py` 側に残っています
...
- stdio builder / registration wiring / runtime bootstrap を外へ出して、`server.py` の責務を少し減らす
- ただし stdio runtime construction の最終分離はまだ途中
...
- stdio runtime construction (`create_runtime(...)`) 側の最終 isolation はまだ途中
...
stdio runtime bootstrap helper split 後も green を維持しています。
...
- ワークツリー上にはこのコミット群に含めていない他の変更
  ...
  が別途存在している前提でした
- 今回のコミット群は Patch 2 scaffold 関連、test 追加、RPC extraction、stdio responsibility split、stdio builder extraction、stdio runtime bootstrap helper split を中心にしています
...
- `server.py` が直接抱えていた protocol / endpoint / RPC branching / stdio transport / stdio builder / stdio bootstrap helper 責務を少し削減
- MCP module 向け focused test を dedicated file に分離
- scaffold 層の変更に対する安全性と見通しが少し上がった
- 将来 stdio を削除しやすい形に少し寄せられた

まだ未着手に近いこと:
- `create_runtime(...)` に残る stdio runtime construction 分岐のさらなる隔離
...
候補:
1. `create_runtime(...)` に残る stdio runtime construction 分岐をさらに helper 化 / 隔離する
...
特に安全そうなのは:
- stdio runtime construction 分岐の抽出
...
7. `tests/test_server.py` と `tests/test_mcp_modules.py` を合わせて **180 passed**
8. `docs/specification.md` は引き続き触らない
9. まだ compliance claim はしない
10. いまの変更は「authority の移設と scaffold / RPC helper / stdio helper / stdio builder / stdio bootstrap helper 追加」であって、full transport rewrite ではない
11. 最終的には stdio は削除する前提だが、現段階では責務分離を優先している
