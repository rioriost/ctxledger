Patch 2 の scaffold 実装、MCP RPC extraction の一部、stdio responsibility split、そして stdio builder extraction まで進めました。
...
- さらに `build_stdio_runtime_adapter(...)` も `mcp/stdio.py` 側へ移し、stdio registration wiring を `server.py` から一段外へ出しました
