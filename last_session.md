...
- `envrcctl` を使うユーザー向けには、生成した token を `CTXLEDGER_SMALL_AUTH_TOKEN` として managed variable に保存し、`direnv allow` 後にそのまま compose 起動と smoke に使う option も README に書くと親切である
- その場合の説明には、`envrcctl set CTXLEDGER_SMALL_AUTH_TOKEN ...`、必要なら `envrcctl get CTXLEDGER_SMALL_AUTH_TOKEN`、および `direnv allow` 後の起動コマンド列を含める
...
- また、`envrcctl` を併用しているユーザー向けには、生成 token を `.envrc` managed variable として保存する方法と、その前提での compose 起動コマンド列まで書くと、日常運用に乗せやすい
...
5. 追加で、`envrcctl` を使って `CTXLEDGER_SMALL_AUTH_TOKEN` を保存する方法と、その前提での compose 起動コマンド列を Quick Start に書く
