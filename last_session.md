...
4. 実際の Zed などの MCP クライアント上の AI エージェントで、`.rules` に従った workflow-aware な運用が回るか確認する
5. 実運用 workflow を開始または再開したら、以下をこの note に記録する
   - `workspace_id`
   - `workflow_instance_id`
   - `attempt_id`
   - `ticket_id`
6. `tests/test_workflow_service.py` の未 commit 差分を確認し、意図した変更なら commit 対象に含める