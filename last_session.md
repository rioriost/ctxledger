...
- `envrcctl` を使うユーザー向けには、生成した token を `CTXLEDGER_SMALL_AUTH_TOKEN` として secret 保存し、`envrcctl exec -- ...` で compose 起動や smoke に流し込む option も README に書くと親切である
- その説明には、`envrcctl secret set CTXLEDGER_SMALL_AUTH_TOKEN --stdin` と `envrcctl exec -- ...` 付きの起動コマンド列を含める
- ただし、検証の結果 **Zed は MCP 設定ファイル中の環境変数を展開しない** ため、Zed の `YOUR_TOKEN_HERE` は環境変数参照ではなく、取得した実 token を貼り付ける必要がある
...
- また、`envrcctl` を併用しているユーザー向けには、生成 token を secret として保存し、`envrcctl exec -- ...` で compose 起動や smoke に注入する方法まで書くと、日常運用に乗せやすい
- ただし、Zed は MCP 設定ファイル中の環境変数を展開しないため、`envrcctl` は **AI エージェントや shell 実行時の参照には有効** でも、Zed の MCP 設定 JSON 中の `YOUR_TOKEN_HERE` を自動置換する用途には使えない
- README 中の `envrcctl` 説明は、冗長にならないよう簡潔に保つ方がよい
...
5. README に、**Zed は MCP 設定ファイル中の環境変数を展開しない** ことと、`envrcctl` 利用時は `envrcctl secret get CTXLEDGER_SMALL_AUTH_TOKEN` などで取得した実 token を `YOUR_TOKEN_HERE` へ貼り付ける必要があることを追記する
6. 実際の Zed などの MCP クライアント上の AI エージェントで、`.rules` に従った workflow-aware な運用が回るか確認する
7. 実運用 workflow を開始または再開したら、以下をこの note に記録する
   - `workspace_id`
   - `workflow_instance_id`
   - `attempt_id`
   - `ticket_id`
8. `tests/test_workflow_service.py` の未 commit 差分を確認し、意図した変更なら commit 対象に含める