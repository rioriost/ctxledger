...
- `workflow_candidate_ordering` は verify / projection freshness を含む workflow freshness signal ベースの candidate 並び替えまで行うようになりました。
...
1. `src/ctxledger/memory/service.py`
   ...
   - verify / projection freshness として以下を lookup できるようにした
     - `latest_verify_report_created_at`
     - `latest_projection_canonical_update_at`
     - `latest_projection_successful_write_at`
   ...
   - `workflow_candidate_ordering` の details には以下を返す
     ...
     - `final_candidate_ids`
     - `candidate_signals`
   - `candidate_signals` により、各 workflow candidate の checkpoint / verify / projection / episode / attempt / workflow freshness snapshot を観測できる
...
2. `tests/test_coverage_targets.py`
   - workflow-instance scope expectation に verify / projection freshness fields を追加
   - non-workflow-instance scope expectation を verify / projection freshness を含む `workflow_freshness_signals` basis に更新
   - intersection scope expectation に verify / projection freshness fields を追加
   - checkpoint freshness が episode recency より優先されるケースを維持
   - checkpoint tie のとき verify freshness が優先されるケースを追加
   - checkpoint / verify tie のとき projection freshness が優先されるケースを追加
...
## 今回の commit
- `Expand memory_get_context freshness signals` (`ad8b3f1`)
...
- `memory_get_context` には以下が入っている状態です。
  ...
  - verify / projection freshness を含む workflow freshness signal ベースの workflow candidate ordering
- 対象変更は commit 済みです。
...
- `projection_open_failure_count` も details に出ますが、現時点では ordering key には使っていません。
...
- `memory_get_context` は
  ...
  - workspace/ticket intersection narrowing
  - candidate ordering details
  - verify / projection freshness を含む workflow freshness signals による candidate ordering
...
1. freshness ordering の次段を改善する
   - completion / terminality / resumable signal の併用
   - projection failure count を ordering に使うか検討する
   - checkpoint / verify / projection freshness の重みづけ見直し
...
## 次回再開時の一言メモ
- `memory_get_context` は verify / projection freshness を含む workflow freshness signal ベースの candidate ordering まで実装済み
...
- `candidate_signals` には `latest_verify_report_created_at` / `latest_projection_canonical_update_at` / `latest_projection_successful_write_at` / `projection_open_failure_count` も入る
- focused pytest は `13 passed`、前回の broad targeted pytest は `427 passed`
- commit `ad8b3f1` まで完了
- 次は completion / terminality 系 signal 追加か、context assembly 改善に進むのが自然