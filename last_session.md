...
- その場合の説明には、`envrcctl secret set CTXLEDGER_SMALL_AUTH_TOKEN --stdin`、必要なら `direnv allow`、および `envrcctl exec -- ...` 付きの起動コマンド列を含める
...
5. 追加で、`envrcctl secret set` で `CTXLEDGER_SMALL_AUTH_TOKEN` を保存し、`envrcctl exec -- ...` 前提で compose 起動や smoke を実行する方法を Quick Start に書く
