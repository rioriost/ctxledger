Patch 2 の scaffold 実装、MCP RPC extraction の一部、そして stdio responsibility split まで進めました。
...
stdio responsibility split 後も green を維持しています。
...
- ワークツリー上にはこのコミット群に含めていない他の変更
  ...
  が別途存在している前提でした
- 今回のコミット群は Patch 2 scaffold 関連、test 追加、RPC extraction、stdio responsibility split を中心にしています
...
- `server.py` が直接抱えていた protocol / endpoint / RPC branching / stdio transport 責務を少し削減
- MCP module 向け focused test を dedicated file に分離
- scaffold 層の変更に対する安全性と見通しが少し上がった
- 将来 stdio を削除しやすい形に少し寄せられた

まだ未着手に近いこと:
- stdio builder (`build_stdio_runtime_adapter(...)`) 自体のさらなる外出し
...
候補:
1. `build_stdio_runtime_adapter(...)` と stdio registration wiring をさらに helper/module 化する
...
5. `tests/test_server.py` と `tests/test_mcp_modules.py` を合わせて **180 passed**
6. `docs/specification.md` は引き続き触らない
7. まだ compliance claim はしない
8. いまの変更は「authority の移設と scaffold / RPC helper / stdio helper 追加」であって、full transport rewrite ではない
9. 最終的には stdio は削除する前提だが、現段階では責務分離を優先している
