final project-wide verification の完了メモとして、この session note を残します。patch 1A の config/orchestration HTTP-only 化、patch 1B の server/module/CLI cleanup、patch 1C の legacy stdio module 削除、patch 1D の protocol/introspection/docs cleanup、patch 1E の plan/review/API wording 整理、env example の HTTP-only 化を前提にして、現時点では **stdio removal は実質完了、project-wide baseline も確認済み** です。

このセッション終端で引き継ぐべき要点:

- active runtime / transport semantics は HTTP-only
- stdio concrete implementation は source tree から削除済み
- public façade / helper layers / main docs / env examples は HTTP-only wording と behavior にほぼ整合
- primary source / tests / docs の major cleanup は完了
- 直近の確認済み focused baseline は:
  ...
  - **204 passed**
- このセッションで project-wide verification を実施:
  - `pytest -q`
  - **256 passed**
- 現在の open question は stdio removal そのものではなく、
  - already-proven minimal HTTP MCP path が `v0.1.0` acceptance として十分か
  - `resources/list` / `resources/read` を含む broader HTTP MCP surface proof を追加で要求するか
  の整理
...
## 2. confirmed verification baselines
...
source / tests / primary docs / env examples の stdio removal はかなり終わっています。ここから見る価値があるのは主に residual historical docs と final acceptance framing です。
...
2. final acceptance boundary clarification
...
- project-wide verification も green
- 残タスクは mostly documentation / evidence / release framing
...
- stdio removal は **effectively complete**
- active runtime / transport semantics は HTTP-only
- stdio concrete implementation は source tree から削除済み
...
もし最後の documentation sweep や acceptance framing まで進める場合の commit 候補例:
