Patch 2 の scaffold 実装、MCP RPC extraction の一部、stdio responsibility split、stdio builder extraction、stdio runtime bootstrap helper split、そして stdio runtime construction split まで進めました。
...
- `build_stdio_runtime_adapter(...)` を `mcp/stdio.py` 側へ移し、stdio registration wiring を `server.py` から一段外へ出しました
- `run_server(...)` に残っていた
  - 単体 stdio runtime 判定
  - composite runtime から stdio runtime を探索する処理
  を `run_stdio_runtime_if_present(...)` へ寄せました
- `create_runtime(...)` に残っていた
  - `server is not None` のときは `build_stdio_runtime_adapter(server)`
  - そうでなければ `StdioRuntimeAdapter(settings)`
  という stdio runtime construction 分岐も `build_stdio_runtime(...)` へ寄せました

意図:
- stdio を今すぐ削除するのではなく、将来消しやすい塊へまとめる
- `server.py` に残る stdio 固有責務を減らす
- HTTP と stdio の transport 境界を少し見えやすくする
- stdio 削除前に、builder / wiring / runtime / server / bootstrap / runtime construction を段階的に分離する
...
- `run_server(...)` に残っていた stdio-specific runtime 判定 / execution 分岐を `run_stdio_runtime_if_present(...)` へ委譲する形へ変更
- `create_runtime(...)` に残っていた stdio-specific runtime construction 分岐を `build_stdio_runtime(...)` へ委譲する形へ変更
...
- stdio builder / registration wiring / runtime bootstrap / runtime construction を外へ出して、`server.py` の責務を少し減らす
- ただし transport orchestration 全体の最終分離はまだ途中
...
- stdio runtime bootstrap helper split
- stdio runtime construction split
...
- transport orchestration 全体の isolation はまだ途中
...
stdio runtime construction split 後も green を維持しています。
...
- `6bcbfaf`
- `Extract stdio runtime bootstrap helpers`

## 注意
- ワークツリー上にはこのコミット群に含めていない他の変更
  （例: `src/ctxledger/db/postgres.py` や `docs/plans/*` など）
  が別途存在している前提でした
- 今回のコミット群は Patch 2 scaffold 関連、test 追加、RPC extraction、stdio responsibility split、stdio builder extraction、stdio runtime bootstrap helper split、stdio runtime construction split を中心にしています
...
- `run_server(...)` に残っていた stdio runtime bootstrap helper を `mcp/stdio.py` に抽出
- `create_runtime(...)` に残っていた stdio runtime construction 分岐を `mcp/stdio.py` に抽出
- `server.py` が直接抱えていた protocol / endpoint / RPC branching / stdio transport / stdio builder / stdio bootstrap helper / stdio runtime construction 責務を少し削減
...
- `server.py` に残る transport orchestration 全体のさらなる薄型化

## 次にやること
次は Patch 2 の仕上げ、または Patch 3 相当の整理に入るのが自然です。

候補:
1. HTTP/stdio で共有する transport orchestration 境界をさらに整理する
...
特に安全そうなのは:
- HTTP/stdio で共有する runtime orchestration helper の抽出
- `test_mcp_modules.py` の fake runtime / settings helper の整理
- stdio 削除前提で、`server.py` から transport-specific startup orchestration をさらに隔離すること
...
6. `run_stdio_runtime_if_present(...)` / `find_stdio_runtime(...)` の helper split も入った
7. `build_stdio_runtime(...)` による stdio runtime construction split も入った
8. `tests/test_server.py` と `tests/test_mcp_modules.py` を合わせて **180 passed**
9. `docs/specification.md` は引き続き触らない
10. まだ compliance claim はしない
11. いまの変更は「authority の移設と scaffold / RPC helper / stdio helper / stdio builder / stdio bootstrap helper / stdio runtime construction helper 追加」であって、full transport rewrite ではない
12. 最終的には stdio は削除する前提だが、現段階では責務分離を優先している
