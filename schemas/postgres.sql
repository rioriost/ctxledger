...
CREATE TABLE IF NOT EXISTS projection_failures (
  projection_failure_id UUID PRIMARY KEY,
  ...
  CONSTRAINT projection_failures_projection_type_valid
    CHECK (projection_type IN ('resume_json', 'resume_md')),
  CONSTRAINT projection_failures_target_path_not_empty
    CHECK (btrim(target_path) <> ''),
  CONSTRAINT projection_failures_error_message_not_empty
    CHECK (btrim(error_message) <> ''),
  CONSTRAINT projection_failures_status_valid
    CHECK (status IN ('open', 'resolved', 'ignored')),
  CONSTRAINT projection_failures_retry_count_non_negative
    CHECK (retry_count >= 0),
  CONSTRAINT projection_failures_resolved_at_after_occurred_at
    CHECK (resolved_at IS NULL OR resolved_at >= occurred_at)
);

CREATE INDEX IF NOT EXISTS idx_projection_failures_workspace_occurred_desc
  ON projection_failures (workspace_id, occurred_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_projection_failures_status
  ON projection_failures (status);

-- ---------------------------------------------------------------------------
-- Projection state
-- Tracks freshness and latest successful writes for each projection artifact.
-- ---------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS projection_states (
  projection_state_id UUID PRIMARY KEY,
  workspace_id UUID NOT NULL REFERENCES workspaces(workspace_id) ON DELETE CASCADE,
  workflow_instance_id UUID REFERENCES workflow_instances(workflow_instance_id) ON DELETE CASCADE,
  projection_type TEXT NOT NULL,
  target_path TEXT NOT NULL,
  ...
  last_canonical_update_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),

  CONSTRAINT projection_states_projection_type_valid
    CHECK (projection_type IN ('resume_json', 'resume_md')),
  CONSTRAINT projection_states_target_path_not_empty
    CHECK (btrim(target_path) <> ''),
  CONSTRAINT projection_states_status_valid
    CHECK (status IN ('fresh', 'stale', 'missing', 'failed'))
);

CREATE UNIQUE INDEX IF NOT EXISTS idx_projection_states_workspace_projection_type
  ON projection_states (workspace_id, projection_type);
